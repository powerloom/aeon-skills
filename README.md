...
2. **Deterministic catch-up** — `fetch-bds-epochs.py` walks `GET /mpp/snapshot/allTrades/{block}` from cursor+1 to chain tip (up to **10** snapshots/run). No tip-only sampling, no ignored `from_epoch` query hacks.
